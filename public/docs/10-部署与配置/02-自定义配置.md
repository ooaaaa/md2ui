...
在文档目录下创建 `mdpage.config.js` 或 `.mdpagerc.json`：
...
export default {
  ...
  folderExpanded: true,
  themeColor: '#3eaf7c',
  github: 'https://github.com/xiaoyaodev/mdpage',
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| title | string | `'mdpage'` | 站点标题，显示在顶部 Logo 旁 |
...
1. 内置默认值
2. 配置文件（mdpage.config.js 或 .mdpagerc.json）
3. 命令行参数（如 `-p 8080`）
