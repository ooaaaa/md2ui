# 自定义配置

验证用户自定义配置的加载和生效。

## 配置文件

在文档目录下创建 `md2page.config.js` 或 `.md2pagerc.json`：

```js
// md2page.config.js
export default {
  title: '我的文档站',
  port: 8080,
  folderExpanded: true,
  themeColor: '#3eaf7c',
  github: 'https://github.com/xiaoyaodev/md2page',
  footer: 'Copyright © 2025'
}
```

## 配置项

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| title | string | `'md2page'` | 站点标题，显示在顶部 Logo 旁 |
| port | number | `3000` | 开发服务器端口 |
| folderExpanded | boolean | `false` | 文件夹默认是否展开 |
| themeColor | string | `'#3eaf7c'` | 主题色，影响全局强调色 |
| github | string | `''` | GitHub 仓库链接，显示在顶部 |
| footer | string | `''` | 页脚内容 |

## JSON 格式

```json
{
  "title": "我的文档站",
  "port": 8080,
  "folderExpanded": true,
  "themeColor": "#3eaf7c",
  "github": "https://github.com/xiaoyaodev/md2page",
  "footer": "Copyright © 2025"
}
```

## 优先级

配置合并优先级（从低到高）：

1. 内置默认值
2. 配置文件（md2page.config.js 或 .md2pagerc.json）
3. 命令行参数（如 `-p 8080`）

## 验证要点

1. 创建配置文件后重启服务，配置应生效
2. 修改 title 后页面标题应更新
3. 修改 themeColor 后主题色应变化
4. 命令行 `-p` 参数应覆盖配置文件中的 port
